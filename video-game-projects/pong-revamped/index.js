(function(window, createjs, opspark, _) {
  ...
  function update(event) {
    ...
    if (ball.y < 0){//makes sure top of the ball does not pass the top of the screen 
      //causes ball to travel in the opposite y direction
    ...
    //Checks if ball colides with
    if (ball.x < (paddlePlayer.x + paddlePlayer.width) && (ball.y + ball.height) > paddlePlayer.y && ball.y < (paddlePlayer.y + paddlePlayer.height)){//checks if the left of the ball passes right edge of player paddle, and if the ball y value is in the range of the player y value
      ball.xVelocity *= -1;
    }
    if ((ball.x + ball.width) > paddleCPU.x && (ball.y + ball.height) > paddleCPU.y && ball.y < (paddleCPU.y + paddleCPU.height)){//checks if the right of the ball passes the left edge of the CPU paddle, and if the ball y value is in the range of the CPU y value
      ball.xVelocity *= -1;
    }
  }
